...
const searchStatusSection = document.getElementById('search-status');
const searchStatusText = document.getElementById('search-status-text');
const resultsFoundSection = document.getElementById('results-found');
const resultsFoundText = document.getElementById('results-found-text');
...
function initializeApp() {
    ...
    viewResultsBtn.addEventListener('click', () => {
        resultsFoundSection.style.display = 'none';
...
// Função para lidar com o blur do campo de CEP
...
async function fetchAddressFromCEP(cep) {
    ...
    try {
        ...
        currentAddress = data;
        addressText.textContent = `${data.localidade}, ${data.uf}`;
...
function handleSearch() {
    if (!currentAddress) {
        showNotification('Por favor, digite um CEP válido e aguarde a busca do endereço.', 'warning');
        cepInput.focus();
        return;
    }

    showSearchStatus(`Buscando peneiras próximas a ${currentAddress.localidade}, ${currentAddress.uf}...`);

    // Simular delay de busca
    setTimeout(() => {
        searchPeneiras(currentAddress);
    }, 3000);
}

// Função para mostrar o status da busca
function showSearchStatus(message) {
    document.getElementById('inicio').style.display = 'none';
    searchStatusText.textContent = message;
    searchStatusSection.style.display = 'block';
    searchStatusSection.scrollIntoView({ behavior: 'smooth' });
}

// Função para buscar peneiras
function searchPeneiras(address) {
    try {
        // Simular geocodificação da localização digitada
        const userCoords = geocodeLocation(address.localidade);
        ...
        showResultsFound(currentResults.length, `${address.localidade}, ${address.uf}`);
...
// Função para mostrar a seção de resultados encontrados
function showResultsFound(count, location) {
    resultsFoundText.textContent = `${count} Peneiras encontradas próximas a você`;
    resultsLocationText.textContent = `em ${location}`;
    resultsFoundSection.style.display = 'block';
    resultsFoundSection.scrollIntoView({ behavior: 'smooth' });
}
...
// O resto do código (funções de utilidade, animações, etc.) permanece o mesmo...
        ...
        function(position) {
            ...
            reverseGeocode(userLocation.lat, userLocation.lng)
                .then(address => {
                    ...
                    currentAddress = { localidade: address.split(',')[0], uf: address.split(',')[1].trim() };
